...
>
> Spec owner: jeff | Version: 1.0 | Date: 2026-04-07
>
...
- DB = runtime SQLite store, currently `ghostlink_v2.db` in `backend/app.py`. Older ops/backup code still references `ghostlink.db` and must be reconciled before these schema extensions are treated as implementation-ready.
...
New SQLite table `tasks` in the runtime SQLite DB [NEW]:
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    trace_id        TEXT    DEFAULT NULL,              -- generated by Phase 3 task/tracing flow and propagated
...
- MCP tool `set_thinking` [EXISTS `backend/mcp_bridge.py:1236`]: agents can set their own thinking level.
...
2. **Enable/disable flow:**
   ...
   - **Per-agent toggle**: When scoped to a specific agent, calls the existing `/api/skills/agent/{agent_name}` endpoints, not `/api/agents/{name}/skills`.
...
| `GET` | `/api/skills/agent/{agent_name}` | Get enabled skills for agent |
| `POST` | `/api/skills/agent/{agent_name}/toggle` | Toggle a skill for an agent |
| `POST` | `/api/skills/agent/{agent_name}/config` | New endpoint if per-agent skill config editing is exposed directly |
...
- `tyson`: Verify/create missing skill API endpoints in the skills routes rather than assuming `/api/agents/{name}/skills` already exists
...
For **tmux-based agents** (runner='tmux'): this polling loop does **not** exist today in `backend/wrapper.py`. It must be added as new logic. The intended behavior is: check for `.cancel_*` files in the agent's data directory, send Ctrl+C via `tmux send-keys`, inject a system message, then delete the signal file.
...
New SQLite table `audit_events` in the runtime SQLite DB [NEW]:
...
**Audit note:** `DataManager.save_retention()` currently only persists four keys:
- `enabled`
- `max_age_days`
- `delete_attachments`
- `delete_memories`

Any audit-specific fields would currently be dropped silently until that allowlist is extended in code.
...
- Phase 1A provides stable `agent_id`; `trace_id` is introduced by the Phase 3 task/tracing model and propagated from there.
...
#### Trace Event Table [NEW in the runtime SQLite DB]
...
| T3-06 | Toggle skill off for agent in Skills Center | `GET /api/skills/agent/{agent_name}` no longer includes that skill ID |
...
New SQLite table `checkpoints` in the runtime SQLite DB [NEW]:
...
CREATE TABLE IF NOT EXISTS checkpoints (
    ...
    agent_id        TEXT    DEFAULT NULL,            -- Phase 1A identity record FK
    agent_name      TEXT    NOT NULL,
    session_id      TEXT    DEFAULT NULL,            -- runtime session identifier if available
...
2. **Agent reads pause signal**: Same mechanism as cancel signal (Section 3.6). On next MCP tool call, the bridge checks for `.pause_*` files. If found, returns: `"Task {task_id} has been paused by the operator. Wait for resume signal. Do not continue work on this task."` For tmux agents, the wrapper pause polling/injection path is **new work**, not an existing behavior.
...
New SQLite table `artifacts` in the runtime SQLite DB [NEW]:
...
Phase 3 (operator control plane) -- this spec
    ...
    |   3.8 Tracing (depends on 3.1 for task_id and on stable Phase 1A `agent_id`)
    |
    v
